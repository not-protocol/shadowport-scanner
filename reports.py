"""
reports.py — ShadowPort Scanner v1.3.0
Save scan results: TXT, JSON, XML, HTML (with risk + service intel).
...
def _ensure_dirs():
    """
    Create reports/ and logs/ inside the project directory.
    Fix permissions if the folder was previously created by root.
    """
    for d in (REPORTS_DIR, LOGS_DIR):
        os.makedirs(d, exist_ok=True)
        try:
            # Make sure the current user can write here
            ...
            pass  # If we can't chmod, we'll catch the write error below
...
<style>
  ...
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .risk-bar-wrap{{margin-top:1rem}}
  .risk-bar-bg{{background:#1a2740;border-radius:4px;height:18px;position:relative}}
  .risk-bar-fill{{height:18px;border-radius:4px;transition:width .5s}}
  .risk-label{{font-size:.85rem;margin-top:.4rem}}
...
def save_report(scan_data: dict, fmt: str = "txt") -> str | None:
    ...
    Always writes to REPORTS_DIR (inside project folder) to avoid permission issues.
    Supported: txt, json, xml, html
    """
    _ensure_dirs()
    fmt      = fmt.lower().strip()
    ts       = _timestamp()
    ...
    try:
        if fmt == "txt":
            ...
            content = json.dumps({
                "meta": {"tool": f"{TOOL_NAME} v{VERSION}", "generated": datetime.now().isoformat()},
                "scan": scan_data,
            }, indent=2)
        elif fmt == "xml":
            ports    = scan_data.get("ports", [])
...
<shadowport_scan>
  <meta><tool>{TOOL_NAME} v{VERSION}</tool><generated>{datetime.now().isoformat()}</generated></meta>
  <host>
    ...
    <risk score="{risk.get('score',0)}" label="{risk.get('label','')}"/>
    <ports>{port_xml}</ports>
  </host>
</shadowport_scan>
"""
        elif fmt == "html":
        ...
        # Make report readable by all users (fixes sudo-created file issues)
        try:
        ...
        print_error(f"Permission denied writing report: {exc}")
